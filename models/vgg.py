...
class VGG_INVERSE(nn.Module):
    ...
    def __init__(self, list_tuple, num_classes=10, init_weights=True, on_cuda=True):
        ...
        self.avgpool_inverse = nn.ConvTranspose2d(512, 512, 3, 1, 1)
        self.linear_layers = nn.ModuleList([nn.Sequential(nn.Linear(num_classes, 4096), nn.ReLU(True), nn.Dropout()), nn.Sequential(nn.Linear(4096, 4096), nn.ReLU(True), nn.Dropout()), nn.Linear(4096, 512 * 1 * 1)])
